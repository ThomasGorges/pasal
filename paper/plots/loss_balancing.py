...
plt.savefig('../output/plots/fig7.pdf')

# Appendix plot
plot_order = ['lambda_history', 'lambda_history_scaled', 'l', 'p', 'r']
num_plots = len(plot_order)

fig, ax = plt.subplots(len(model_paths), num_plots, sharex=True)
fig.subplots_adjust(hspace=0)
fig.set_figwidth(12)
...
for model_path in model_paths:
    ...
    with open(model_path + 'loss_history.json') as f:
    ...
    for k in plot_order:
        ...
        if k == 'l_hat':
            ax[y][x].plot(np.arange(len(loss_balancing_history[k])), loss_balancing_history[k], 'black', label=label_prefix + 'l_hat')
        elif k == 'l':
            heads = loss_history['train'].keys()

            for head in heads:
                head_name = head_to_name(head)
                
                ax[y][x].plot(np.arange(len(loss_history['train'][head])), loss_history['train'][head], label=f'{label_prefix}{head_name}')
        else:
            heads = loss_balancing_history[k].keys()
            
            for head in heads:
                head_name = head_to_name(head)
                
                ax[y][x].plot(np.arange(len(loss_balancing_history[k][head])), loss_balancing_history[k][head], label=f'{label_prefix}{head_name}')

        ax[y][x].xaxis.set_major_locator(MultipleLocator(10))
        if y == 0:
            if k == 'lambda_history':
                ax[y][0].set_title(r'$\lambda$')
            elif k == 'lambda_history_scaled':
                ax[y][1].set_title(r'Scaled $\lambda$ with #samples')
            elif k == 'l':
                ax[y][2].set_title(r'L')
            elif k == 'p':
                ax[y][3].set_title(r'Learning progress p')
            elif k == 'r':
                ax[y][4].set_title(r'Rel. inv. training ratio r')

        x += 1
        
        ylabel = r'$(\alpha=$' + f'{alpha:.2f}' + r', $\beta=$' + f'{beta:.2f})'
        if alpha == 0.0 and beta == 0.0:
            ylabel = 'Constant loss balancing factors'
        
        ax[y][0].set_ylabel(ylabel)
    
    y += 1

fig.legend(loc='upper center', title='Dataset', ncols=len(loss_balancing_history['lambda_history']), fancybox=False, shadow=True, bbox_to_anchor=(0.5, 1.1))

for a in ax[:, :].flat:
    a.set(xlabel='Epoch')

plt.tight_layout()
plt.savefig('../output/plots/fig7_appendix.pdf')
